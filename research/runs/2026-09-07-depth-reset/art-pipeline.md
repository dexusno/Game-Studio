...
**Updated trial recommendation: compare Meshy 7, Rodin Gen-2.5, Tripo P2.0 Preview and TRELLIS.2-4B, then finish in Blender and inspect in actual UE5.8.** Klaus requested current user experience, commercial leaders and latest open offerings, including rented GPU and paid API routes. [September 8 version and experience review](ai-3d-current-review.md) supersedes the website-only/Rodin-first selection below. No winner has been tested for this game. Rodin's [official API](https://docs.hyper3d.ai/en/api-specification/rodin-gen2-5) supplies labeled views, faithful guidance, bounds and PBR; explicitly request faithful because the current default is creative. Shield joints and moving parts must remain deliberately authored.

[Tripo H3.1](https://developers.tripo3d.ai/en/models) remains the high-detail family, with restrictions on combined output modes; the newer [P2.0 Preview](https://www.tripo3d.ai/blog/tripo-p2-0-preview), released August 19, targets editable native quads and multiview geometry. [Meshy 7](https://docs.meshy.ai/en/api/multi-image-to-3d) is current, while Meshy T2 is its separate Smart Topology model. Current developer reports demonstrate useful assets plus finishing work; none proves correct deformation for our cyborg or unusual creatures. No model has been generated for this game.

Rodin's [trial FAQ](https://hyper3d.ai/features/image-to-3d) advertises free credits, real model export and no card requirement. Root observed Start Creating leads to login; actual allowance and export remain unverified. Its direct [pricing](https://hyper3d.ai/pricing?lang=en) lists Creator US$30/month and Business API US$120/month, but this is not the only API route: [fal Rodin Gen-2.5](https://fal.ai/models/fal-ai/hyper3d/rodin/v2.5) lists US$0.40/generation, plus US$0.80 for HighPack. Reseller settings/entitlements must be compared with the current direct product. Hyper3D's [terms](https://hyper3d.ai/legal/terms) mention a two-month minimum; free FAQ/commercial-use language differs. [Meshy](https://www.meshy.ai/pricing) and [Tripo commercial-use documentation](https://www.tripo3d.ai/help/privacy-policy/how-to-use-tripo-models-commercially) also require checking the actual plan/export rights. No account or purchase was created. These are observed offers, not checkout commitments; older H3 generation prices do not price P2.0 Preview.
...
[Microsoft TRELLIS.2](https://github.com/microsoft/TRELLIS.2) is the first downloadable comparison candidate: upstream specifies Linux and at least 24GB NVIDIA VRAM. The machine has an RTX4090; rented Linux GPUs are also in scope for consideration, with spending still unapproved. Setup/output have not been tested. Its core code/weights are MIT, but the current nvdiffrast dependency has a noncommercial use restriction; resolve the exact runtime path before adopting a commercial pipeline. The [latest review](ai-3d-current-review.md) distinguishes newer rigging/topology research, restrictive downloadable licenses and announced models without weights.
