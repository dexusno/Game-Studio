...
- For project work, run `python scripts/studio.py context` before broad reading. It discovers games and research from existing records. The current cyborg/worlds beta resumes with `python scripts/studio.py context game:dreambound`; dreambound is an internal identifier, not the final title. Its design archive remains under opportunity:reactor-raider. Follow an explicit different assignment instead of assuming this is always the active game.
- If a lifecycle hook already supplied the relevant context, use it without repeating the read. Once a task's game/opportunity is explicit, run `python scripts/continuity.py bind TARGET` once to load its handoff and bind that task's automatic resume/compaction context. Rebind only when scope changes. This local pointer is routing, not a second status record.
...
At a milestone that needs a fresh task, run `python scripts/continuity.py handoff TARGET --focus "the next outcome"` after the checkpoint and commit. It prepares the receiving prompt with the exact revision and refuses stale or uncommitted handoffs. Use that prompt when a new task is authorized; do not create tasks on a timer or just because compaction occurred. Hooks never replace agent-written decisions, commit/merge work, or force another test pass.
