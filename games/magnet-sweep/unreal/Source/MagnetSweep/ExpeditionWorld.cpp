...
void FExpeditionWorld::StartSite(int32 SiteIndex, int32 Seed, int32 InitialBattery)
{
...
bool FExpeditionWorld::IsArmSafe() const { return State.bArmBraced || FMath::Fmod(State.WorldTime, 6.f) < 3.7f; }
bool FExpeditionWorld::IsPressSafe() const { return FMath::Fmod(State.WorldTime,5.5f)<3.4f; }
FVector2D FExpeditionWorld::GetArmTip() const
{
    const float Phase=State.bArmBraced?0.f:FMath::Fmod(State.WorldTime,6.f)/6.f;
    return ArmPivot()+FVector2D(-100.f*FMath::Cos(Phase*PI),-95.f*FMath::Sin(Phase*PI));
}
...
bool FExpeditionWorld::ArcPath(int32 Target,const FExpeditionRig& Rig,TArray<int32>& OutPath) const
{
    OutPath.Reset(); const auto* T=FindBody(Target);
    ...
    TArray<int32> Queue={5}; TMap<int32,int32> Parent; Parent.Add(5,INDEX_NONE);
    for(const auto& B:State.Bodies)if(B.bFunctional&&B.Charge>0&&Live(B)&&!Held(B)&&B.Links.IsEmpty()) {Queue.AddUnique(B.Id);Parent.Add(B.Id,INDEX_NONE);}
    bool UsedBridge=false;
    for(int32 I=0;I<Queue.Num();++I)
    {
        const int32 Id=Queue[I];
        if(Id==Target&&Id!=5){for(int32 P=Id;P!=INDEX_NONE;P=Parent[P])OutPath.Insert(P,0);return true;}
        auto Next=ConductiveNeighbors(Id,Rig);
        // A finite optional air-gap edge; exactly one gap may enter this discharge search.
        if(Has(Rig,TEXT("induction_bridge"))&&!UsedBridge)
            for(const auto& B:State.Bodies)if(B.Id!=Id&&Live(B)&&!Held(B)&&B.bConductive&&B.Links.IsEmpty()&&!Next.Contains(B.Id)&&Near(B.Position,FindBody(Id)->Position,80+B.Radius+FindBody(Id)->Radius)) {Next.Add(B.Id);UsedBridge=true;break;}
        for(int32 N:Next)if(!Parent.Contains(N)){Parent.Add(N,Id);Queue.Add(N);}
    }
    // Aiming the input terminal means actuating its remote receiver, not a free successful empty pulse.
    if(Target==5)return ArcPath(4,Rig,OutPath);
    return false;
}
...
FExpeditionPreview FExpeditionWorld::Preview(const FExpeditionCommand& C, const FExpeditionRig& Rig) const
{
    ...
    {
        TArray<int32> Path;
        if (!ArcPath(TargetId, Rig, Path)) return Refuse(TEXT("No conducting path: free the long conductor, place it in the socket, then pulse the terminal"));
        P.BodyIds = Path; P.BatteryCost = 8;
        if (Has(Rig,TEXT("induction_bridge")) && Path.Num() > 1) P.BatteryCost += 4;
        if (State.bCircuitClosed && Target && Target->Role != TEXT("ordinary_terminal")) return Refuse(TEXT("That circuit has already actuated its latch"));
        if (Target && Target->Role == TEXT("ordinary_terminal") && State.bArmBraced) return Refuse(TEXT("The arm is already held clear"));
        break;
    }
    case EExpeditionAction::Winch:
    {
        ...
        {
            bool Weight = false;
            for (const auto& B : State.Bodies) if (B.Id!=Target->Id&&Live(B) && !Held(B) && B.Mass >= 8 && Near(B.Position,CounterweightPad(),55)&&B.Velocity.Size()<45) Weight = true;
    ...
    case EExpeditionAction::ArmRelay:
    {
        ...
        P.BodyIds={TargetId,C.SecondaryId};P.BatteryCost=11;break;
    }
...
FExpeditionResult FExpeditionWorld::Execute(const FExpeditionCommand& C, const FExpeditionRig& Rig)
{
    ...
    case EExpeditionAction::Launch:
    {
        ...
        B->State = EExpeditionBodyState::Available; if(!Orbit)B->Position = InTray(C.Magnet,B->Radius); B->Velocity = (C.Aim-(Orbit?B->Position:C.Magnet)).GetSafeNormal() * (560.f + FMath::Min(B->Mass,12.f)*12.f); B->bLaunched = true; B->bRebounded = false; B->bPunched = false; B->RootAction = State.ActionSerial; B->RecoverDelay = .5f; B->FuseDelay = -1;
    ...
    case EExpeditionAction::Winch:
    {
        ...
        if(B->bAnchored)
        {
            int32 Weight=INDEX_NONE;for(const auto& W:State.Bodies)if(W.Id!=B->Id&&Live(W)&&!Held(W)&&W.Mass>=8&&W.Velocity.Size()<45&&Near(W.Position,CounterweightPad(),55)){Weight=W.Id;break;}
            if(Weight!=INDEX_NONE)
            {
                State.Constraints.RemoveAll([&](const FExpeditionConstraint& K){return K.BodyA==B->Id;});
                FExpeditionConstraint K;K.Kind=Has(Rig,TEXT("walking_gantry"))?EExpeditionConstraintKind::Gantry:EExpeditionConstraintKind::Counterweight;K.BodyA=B->Id;K.BodyB=Weight;K.Anchor=CounterweightPad();K.Offset=FindBody(Weight)->Position-B->Position;State.Constraints.Add(K);
...
bool FExpeditionWorld::CanDispatch(FString& Reason) const
{
    ...
    Reason=TEXT("Release the three physical restraints, then secure the machine"); return false;
}
...
{
    if(Id==11)State.bArmBraced=true;
...
void FExpeditionWorld::MakeCyclone(int32 BodyId)
{
    ...
    else
    {
        ...
        for(int32 Part=0;Part<FMath::Min(6,Saved.SourceIds.Num());++Part)
        {
            ...
            Fragment.Role=TEXT("cyclone_fragment");Fragment.State=EExpeditionBodyState::Available;Fragment.Links.Reset();Fragment.bAnchored=false;Fragment.bLaunched=false;Fragment.bRebounded=false;Fragment.bPunched=false;Fragment.RootAction=Saved.RootAction;Fragment.Quality=Saved.Quality;Fragment.FuseDelay=Part==0&&Saved.FuseDelay>=0?Saved.FuseDelay:-2;Fragment.RecoverDelay=.3f;Fragment.Position=Saved.Position;Fragment.Radius=FMath::Clamp(12.f+Fragment.Mass,15.f,30.f);
...
void FExpeditionWorld::Impact(FExpeditionBody& B, const FVector2D& Normal, float Speed, const FExpeditionRig& Rig)
{
    if (Speed < 75) return;
    ResolveSensor(B.Id);
...
void FExpeditionWorld::Integrate(float Delta, const FExpeditionRig& Rig)
{
    ...
    for (int32 I=0;I<State.Bodies.Num();++I) for (int32 J=I+1;J<State.Bodies.Num();++J)
    {
        ...
        const float InvA=A.bAnchored?0.f:1.f/A.Mass, InvB=B.bAnchored?0.f:1.f/B.Mass;
        if (InvA+InvB<=0) continue;
        const float Closing=FVector2D::DotProduct(A.Velocity-B.Velocity,N);
        if (Closing>40)
        {
            auto Break=[&](FExpeditionBody& Hit,FExpeditionBody& Shot)
            {
                if (!Hit.bBrittle || !Shot.bLaunched || Shot.Mass<4 || Closing<170) return;
                ...
                if(Has(Rig,TEXT("punch_through_collar"))&&!Shot.bPunched) { Shot.bPunched=true; Hit.Velocity=N*160.f; }
                AddEvent(EExpeditionEventKind::Mechanism,TEXT("Physical brace broken by the slug"),Hit.Position,{Hit.Id,Shot.Id});
            };
            Break(B,A); Break(A,B);
            const float Impulse=1.25f*Closing/(InvA+InvB);
            A.Velocity-=N*Impulse*InvA; B.Velocity+=N*Impulse*InvB;
            Impact(A,-N,Closing,Rig); Impact(B,N,Closing,Rig);
    ...
    if(State.HazardCooldown<=0 && !IsPressSafe())
    {
        ...
        {
            if(!Held(B))continue;
            const FVector2D D=B.Position-PressCenter();
            if(FMath::Abs(D.X)>PressHalfSize().X+B.Radius||FMath::Abs(D.Y)>PressHalfSize().Y+B.Radius)continue;
            int32 Interceptor=INDEX_NONE;
            for(int32 Id:State.CycloneIds)if(const auto* Fragment=FindBody(Id);Fragment&&Live(*Fragment)&&Near(Fragment->Position,B.Position,65)&&FMath::Abs(Fragment->Position.X-PressCenter().X)<PressHalfSize().X+Fragment->Radius){Interceptor=Id;break;}
...
#define EB_BOOL(Name) Item->SetBoolField(TEXT(#Name),B.Name)
        ...
        EB_NUM(Radius); EB_NUM(Mass); EB_NUM(Value);EB_NUM(Appraisal); EB_NUM(Quality); EB_NUM(Charge); EB_NUM(FuseDelay); EB_NUM(RecoverDelay); EB_NUM(RootAction);
...
#undef READ_B_BOOL
    ...
    State=MoveTemp(Candidate.State); Obstacles=MoveTemp(Candidate.Obstacles); Events.Reset();PendingCyclones.Reset(); Error.Reset(); return true;
}

bool FExpeditionWorld::CheckInvariants(FString& Error) const
{
    auto Fail=[&](const TCHAR* Why){Error=Why;return false;};
    if(State.Version!=2||State.SiteIndex<0||State.SiteIndex>3||State.Battery<0||State.Battery>120||State.Output<0||State.ActionSerial<0||State.NextBodyId<0||State.NextBodyId>1000000)return Fail(TEXT("Invalid or incompatible world ledger"));
    ...
    FExpeditionWorld Original; Original.StartSite(State.SiteIndex,State.Seed);
