...
| Next round | If the player and any enemies remain in the fight, advance the round, show the surviving enemies' new intentions and return to the claw/baseline-choice step, then loadout. Keep unused resources and parts. The manual Precision use remains spent if already used. Exact start-of-round ordering of status ticks, cooldowns, Shield expiry and upgrade supplies remains open; a repeated OK prompt each round is not yet required. |
...
- **Round boundary:** order cooldown reduction, status effects, Shield expiry, delayed supplies and new intent; whether later rounds repeat the initial OK acknowledgement.
...
The current physical tool is a **claw**. This heading and older catalogue labels retain the legacy magnet terminology; their gathering-yield and next-round roles carry forward. The new craft-to-parts-pool flow must still settle whether these enhancements fit automatically on craft or need an explicit use action.
