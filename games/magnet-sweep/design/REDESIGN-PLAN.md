...
| C — Occasional precision grab | Normal collection is automatic. A precision opportunity is used at selected moments; a first candidate allows the player to choose one turn per fight for it | Lower repetition and a decision about when a targeted supply boost matters | A large all-or-nothing payout could make one miss feel worse than several small misses. Keep a useful targeted minimum |

These can be combined. **Current recommendation:** explore a mechanical claw with baseline supplies, simple material targeting and occasional precision handling. A persistent resource priority can remain selected until the player changes it, avoiding a forced extra selection dialogue every turn. Compare B against B+C before making A a mandatory part of every turn. The claw suits the mixed-scrap fiction; it does not require introducing aluminium as a sixth ingredient or renaming the project now. If timing is retained, show the target on the claw's short movement path or beside it, with one stop input and clear bonus quantities. Target choice is made without a timer; only the brief precision action is timed.
...
**First comparison, proposed only:** automatic targeted collection versus the same baseline and targeting with occasional precision. Keep expected rewards comparable and observe a 30–50-turn session, including ordinary and pressured rounds. Does precision change a crafting plan? Is it still chosen when it pays about the same on average? Does the player start skipping it, resent its delay, or feel forced to maximise it? Keep it only if the action itself remains enjoyable. No playable minigame, graphics, input implementation or change to the 600 recipes has been made in this discussion.

**Next:** choose the gathering candidate and whether manual handling is every turn or occasional; then define the baseline material mix, automatic result and gathering-upgrade/next-round-boost mapping. The other prototype-readiness gaps below remain open.
...
| --- | --- | --- |
| Magnet handling and supply | Controls; how aim, swing, reach and physical contact determine the material haul; yield/weight limits; pile replenishment; what happens after a poor or empty haul; whether intent is visible before gathering | One haul per turn and rear gathering are selected, but they do not specify what skill changes or how many useful crafting choices a haul supports |
| Turn commitment | Exact start-of-turn order; when gathering/crafting/part use stop; whether Fire ends planning and leads directly to enemies; passing without firing; whether an unloaded gun can fire its base damage | These decisions determine the game's controls and every delayed effect. The current design centres on one assembled upcoming shot; extra main shots are not approved |
...
**Proposed starting defaults for discussion:** Fire commits the prepared shot and proceeds to enemy actions; a separate pass ends planning without firing. Start the first balance experiment with the catalogue's resource-only costs and explicit effect rules. Use the listed Shield expiry and fresh-fight cooldowns as provisional test rules if accepted. Choose a memory number and a matching kit before implementing rewards. These recommendations are not owner decisions or proof of balance.

### Presentation and technical preparation

Specify one playable preparation layout: distinguish raw stock, finished reserve, loaded Ammo, activated defence, recipe availability and the combined outcome. Decide which edits are reversible: unloading before Fire is different from refunding a completed craft or undoing an already activated helper command. Keep targets and enemy intentions readable while the recipe selector is open. The outcome preview and action sequence must describe the same result, including multi-enemy hits and delayed effects.

Use the selected rear magnet and 16B/16C cameras, with a representative forge, robot and surroundings for one city. Test the required 20-part shot and 20-part shield readability with a prepared stress case. Choose a practical grouped or physical assembly presentation for this demo, and include magnetic, furnace, firing and impact feedback. The existing generated boards and Blender clip are useful references; actual interactive assets, input, camera behaviour and runtime quality still need proving.
...
**Next discussion:** settle the magnet's effect on the haul and the exact round sequence, then write one complete example fight with starting stock, enemy intentions, attack/defence choices, damage, victory and defeat. Numeric tuning can change during prototype play; these structural rules need explicit temporary answers first.

## How we will work

### Mechanics and scenes Q&A — opening screen first

Known foundation: individual mercenaries/weapons and core recipes; 10–20-fight city waves; one magnet haul per turn; player-assigned furnace crafting from limited recipe memory; cooldown recipes usable again when all counters are removed; recipes without a cooldown ability limited to once per turn unless a permanent upgrade says otherwise; resource/part retention between turns and clearing at fight end; game-long retained recipes/upgrades including memory expansion, fight-start and turn-timed supply grants; core/recipe rewards; Officers; between-fight shops; and mysteries. Preparation uses 16B and action uses 16C. Exact crafting/energy numbers, memory size, cooldown interactions, turn ending and new-game/profile persistence remain open.

Klaus wants to begin at the opening screen and work forward through the player journey. Title Screen leads through Start New Game to Choose Character. He then defined the recipe/part combat and reward loop below. Ask one main question at a time and preserve these answers. The latest prepare-screen requirement describes a single upcoming shot assembled from many parts. The earlier separate End Turn/multiple-shots suggestion is not an accepted rule; exact Fire/turn-ending behaviour remains open.
...
| Preparation and assembly | Selected side view; magnet/scrap gathering behind the player/gun, enemy intent visible; support at least 20 applied parts each in the bullet and shield; show accumulated effects, with physical parts optional; Recipe Memory button opens recipe cards for resource-based crafting; magnet and massive scrap pile; hard part caps remain undecided |
...
**Acquisition is separate from weapon rewards.** Do not place magnet upgrades into the mayor/Officer weapon-reward offers by default. The owner specifies these routes:

| Route | Effect and duration |
| --- | --- |
| Shop-bought magnet upgrade | Upgrade the magnet's gathering yield and/or resource targeting as part of the current game's permanent systems |
| Special recipe | Craft a temporary magnet enhancement for **the next round only**: crafting in round N prepares the enhancement for round N+1, not the current round or an indefinite future round |

Temporary effects last for that one next round. Prices, recipe costs, effect strengths, stacking, exact application timing within the next round and what happens if the fight ends before that round remain to define. No permanent magnet-upgrade crafting or ordinary weapon-reward drop is selected by this requirement.
...
| Recipe | Knowledge stored in limited recipe memory that allows production of a part when affordable, off cooldown and within its permitted uses this round; recipes take the build/reward role of cards and appear in the prepare screen's recipe-card pop-up; crafted parts are the physical ammunition/defence ingredients |
...
| Physical magnet | Suspended magnet and massive scrap pile behind the player/gun, outside the space between gun and enemies; once per turn gathering for the furnace; shop upgrades improve yield and/or bias one or more resource types separately from weapon rewards; special recipes enhance only the next round |
