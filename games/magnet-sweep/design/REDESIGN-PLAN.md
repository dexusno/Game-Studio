...
| Preparation and assembly | Selected side view; support at least 20 applied parts each in the bullet and shield; show accumulated effects, with physical parts optional; Recipe Memory button opens recipe cards for resource-based crafting; magnet and massive scrap pile; hard part caps remain undecided |
...
**Gathering scene:** the prepare screen includes the magnet system and a **massive pile of scrap**. The magnet's gathering yield and resource mix can be upgraded to suit the build, as specified below. Only record these requirements at this stage; no new mock-up, game screen or gameplay implementation is requested.
...
| Preparation view | 16B side view; minimum support for 20 parts per bullet and 20 per shield, accumulated effects visible, physical-part display optional; Recipe Memory button opens recipe cards; hard part limits and exact layout remain open |
...
| Physical magnet | Suspended magnet and massive scrap pile; once per turn gathering for the furnace; shop upgrades improve yield and/or bias one or more resource types separately from weapon rewards; special recipes enhance only the next round |
