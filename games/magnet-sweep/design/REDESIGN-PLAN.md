...
Known foundation: individual mercenaries/weapons and core recipes; 10–20-fight city waves; one magnet haul per turn; player-assigned furnace crafting from limited recipe memory, once per recipe per round unless explicitly enhanced, with recipe-specific cooldowns and possible counter-removal effects; resource/part retention between turns and clearing at fight end; game-long retained recipes/upgrades including memory expansion, fight-start and turn-timed supply grants; core/recipe rewards; Officers; between-fight shops; and mysteries. Preparation uses 16B and action uses 16C. Exact crafting/energy numbers, memory size, cooldown interactions, turn ending and new-game/profile persistence remain open.
...
**Availability each round:** all recipes in memory are accessible for selection without a random draw. Crafting requires sufficient resources, a remaining use this round and no active cooldown. **Each recipe can be crafted only once per round, regardless of whether it has a cooldown**, unless an enhancement or special ability explicitly allows more uses per round. Ordinary recipes without cooldown can therefore be used once every round. Some recipes, particularly stronger ones, activate a cooldown when used for crafting; saving the finished part does not delay that trigger until firing. Increased output from one craft and additional crafting uses per round are separate effects.

**Cooldown example:** crafting a recipe with a two-round cooldown in round 4 makes it unavailable for crafting in rounds 5 and 6, then available again in round 7. Two rounds is an illustrative cooldown, not a universal duration for strong recipes. **An active cooldown means the recipe cannot be used in the current round.** Extra permitted uses per round do not by themselves override that block. The cooldown restricts crafting that recipe; the existing rule permitting storage of crafted parts still applies.

**Cooldown-counter effects:** other recipes may provide an ability that removes a cooldown counter. This changes cooldown state, not the separate count of permitted uses this round; it does not itself grant an extra craft. Exact effect targets, activation timing, counter removal amounts and whether clearing the last counter makes the recipe usable immediately in that round still need specification. The example above describes normal cooldown duration without such effects.
...
| Recipe | Knowledge stored in limited recipe memory that allows production of a part when affordable, off cooldown and within its permitted uses this round; recipes take the build/reward role of cards, not physical ammunition or a card UI |
...
2. The gathered materials are compatible with **many recipes**. The player **assigns resources to an affordable recipe in memory that is off cooldown and has a remaining use this round**, crafting the parts needed most now or preparing a stock for later. A recipe can be crafted once per round unless an enhancement or special ability explicitly increases that allowance. Crafting activates its cooldown if it has one. The furnace does not automatically decide the crafted batch. Recipe costs, ingredient ratios, part yields, specific enhancements and the detailed crafting UI remain open.
...
| Recipe memory | Hard recipe-count limit, exact size later; choose any stored recipe that is affordable, off cooldown and has uses left; one craft per recipe per round unless explicitly enhanced; crafting starts applicable cooldowns and other recipes may remove counters; exchange recipes at capacity; permanent upgrades can expand memory |
...
**Next action: clarify whether recipe cooldowns reset at the start of each fight.** Recipe availability and normal crafting frequency are answered: choose from memory subject to costs, per-round use allowance and cooldown, within a hard capacity that permanent upgrades can expand. Recipes normally allow one craft per round; enhancements or special abilities may grant more uses, and other recipes may remove cooldown counters. Then cover counter-removal timing, mystery counting and the final boss's position in encounter choice. Standalone removal has no current advantage and remains under review: a benefit for free memory or dropping that service are unresolved alternatives. Fight-end reset is answered for stock: clear both raw resources and unused crafted parts, while retained upgrades can create fresh fight-start or turn-timed supplies. Preserve the selected mercenary/city world, 10–20-fight waves, mayor/Officer upgrades, core/credit economy and crafting/rewards. Full gameplay implementation follows an owner-set goal; publication, marketing and purchases remain outside this design discussion.
