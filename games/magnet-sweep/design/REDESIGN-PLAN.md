...
| --- | --- | --- |
| Gathering and supply | Baseline material mix and quantity; automatic result; manual bonus bands and input/commit rules; upgrade mapping; claw or magnet presentation; whether intent is visible before gathering | Automatic normal collection each turn, one optional manual timing grab per fight and rear gathering are selected. Exact supplies must support useful crafting choices without requiring a perfect result |
