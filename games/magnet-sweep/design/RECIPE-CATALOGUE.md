...
**Updated owner rules, 14 September:** round Shield/builds clear after the complete enemy turn, and upgrades may grant fresh Shield next round. Every active cooldown loses one counter at enemy-turn end, including counters added that same round. Cooling effects apply globally without selected recipe targets. Permanent claw upgrades from shops/Officer loot activate on purchase/acceptance; the owner also keeps all 23 temporary gathering recipes. Recipe loot remains zero or one choice from three, with free skipping and memory replacement when full. See [the authoritative clarification](REDESIGN-PLAN.md#round-resets-cooling-and-accepted-loot--14-september-2026). Printed costs, strengths and cooldown counts remain draft tuning; global cooling broadens effects and collapses some earlier target-count distinctions. Shield-retention rows remain proposed exceptions requiring review, not confirmed departures from the reset rule.
...
**Shield:** absorbs incoming damage before HP. By default remaining Shield clears after the complete enemy turn, before the next round. Upgrades may grant fresh Shield at the next round's start. Existing retention rows are unapproved exception candidates; they must not silently override this reset. It cannot fall below zero. An enemy's Shield works the same way when it takes damage; `lose Shield` removes protection and is not HP damage. Paying HP bypasses Shield, must leave at least 1 HP, and is not enemy damage. A part with an extra Heat, Charge, Shield, HP or reserve-part cost cannot be activated without paying that cost.

Old Shield has already cleared at enemy-turn end; apply fresh starting grants at their specified next-round trigger. **Earlier retention proposal, pending a decision:** if retention exceptions are kept, use the highest allowance rather than adding allowances. This is not an accepted exception merely because a draft row mentions it. Printed enemy attack intent means the displayed attack totals, including declared multi-hit damage, before your protection and interception. Changes to an enemy's planned damage update that display.
...
| SH113 | Last Wall | 1 Last Wall Plate | Shield | 3 Iron + 1 Copper + 1 Glass | 3 | Gain 38 Shield. Keep up to 20 remaining Shield at the start of next turn. If at least 10 Shield is retained then, remove 1 cooldown counter from every currently cooling recipe. The retention clause remains an unapproved exception candidate. |
