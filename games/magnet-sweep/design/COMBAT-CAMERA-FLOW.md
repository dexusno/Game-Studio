...
| --- | --- | --- |
| Prepare — 2.5D side view | Read enemy intentions, use the magnet once to gather materials, assign resources to affordable recipes available under their cooldown or once-per-turn rules, then arrange crafted/retained parts into shot and defence | Raw-resource reserves versus crafted-part reserves; recipe-memory usage/capacity; recipe costs and crafting output; remaining cooldown counters for cooldown recipes, or uses remaining this turn for recipes without cooldown; immediate readiness after the last counter is removed; available versus committed parts; energy; targets; expected effects before Fire |
