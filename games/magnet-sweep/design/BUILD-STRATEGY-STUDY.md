...
**Later owner-defined rules, 13 September 2026:** [the current redesign plan](REDESIGN-PLAN.md#owner-defined-recipe-and-part-system) specifies character-specific starting weapons/core recipes, one magnet haul per turn, player-assigned furnace crafting, ammo/defence/modifier parts, and retention of unused raw resources/parts between turns. **Both consumable pools clear at fight end; retained recipes and permanent weapon upgrades persist through the current game.** Upgrades may expand recipe memory or grant fresh starting or turn-timed resources/ready parts. Recipe memory has a hard capacity, exact size later; each round the player chooses any stored recipe that is off cooldown, affordable and has a remaining use. Each recipe allows one craft per round unless an enhancement or special ability explicitly grants more. Active cooldown blocks use even with extra uses available; other recipes may remove cooldown counters. Crafting starts applicable recipe cooldowns; a two-round cooldown blocks the next two rounds. Recipe rewards normally offer one of three or a skip, with exchange available at full memory. Materials fit many recipes, and the player chooses what to craft now or later. Standalone removal has no current advantage; Klaus leaves either adding a benefit for free memory or dropping the service open. Exact costs, cooldown-counter effect timing, cooldown handling at fight boundaries, turn ending and new-game/profile persistence remain open. This update governs over earlier direct-component-gathering, multiple-haul and persistence proposals below; research examples are not implemented or numerically approved.
