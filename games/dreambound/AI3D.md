...
For larger assets on the24GB4090, `--decode-row-chunk 262144` now bounds temporary normalization, final projection and upsample residual allocations, and releases completed decoder convolution caches. It preserves the pinned inference blocks, weights, subdivision values, channel normalization and requested voxel resolution; it is inference-only. An independent source review found no semantic or aliasing mismatch, without claiming numerical validation. RootRock completed at genuine1536/4K after the original decode and three resumed variants failed on large allocations. Successful output is `bellroot-kit-v1/root-rock/source-resumed4`, with the original sampling checkpoint preserved in `root-rock/source/latents.pt`. This fixes a demonstrated local generation failure; it is not a new benchmark or evidence that arbitrary scene-scale inputs fit the GPU.
