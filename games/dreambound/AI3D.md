...
## Bellroot playable art kit — September 10
Klaus authorized a graphics upgrade toward anchor B using the working TRELLIS pipeline. Three new original RGBA references live in `art/references/bellroot-v1`: BellTree, Cloister and RootRock. They join the owner-liked repaired V2 waymarker. The scope is the existing three procedural courts; it does not establish a new biome or alter encounters.

Generate each reference using `Start-AI3D.ps1 -Tool TrellisTrial` with `--quality 1536_cascade --max-num-tokens 98304 --seed 42 --export-faces 1000000`. Outputs retain the full decoded source and original 4K PNG PBR maps. The first tree run failed during decoding with a CUDA driver error; the runner now saves `latents.pt` before decoding and includes the traceback on failure. `--resume-latents <exact-local-latents.pt>` can decode the same image/model/seed/resolution into a new output directory without repeating sampling. No resolution fallback is accepted.

Prepare a completed source with background Blender: `--threads 4 --python-exit-code 1 --python scripts/prepare_trellis_kit.py -- --asset <BellTree|Cloister|RootRock> --source <textured.glb> --output <finished>`. It scales uniformly, grounds the pivot, preserves maps, supplies the shared dual-UV material contract and authors separate UCX collision pieces. The arch opening is measured for placement; it is optional scenery beside the protected route.

Import using the full Unreal editor, not the content commandlet: `-ExecutePythonScript=<game>/scripts/import_trellis_kit.py -TrellisKitConfig=<private-config.json>`. Config entries contain `name`, `fbx`, `base_color` and `metallic_roughness`; supported names are BellTree, Cloister, RootRock and Waymarker. The importer writes only `/Game/Art/Trellis`, requires Nanite geometry/two UV sets/authored convex collision, and records `Saved/TrellisArt/import.json`. New scenery is instanced in `DBRecoveryScene.cpp`; original art is a missing-import fallback. Large generated outputs and imported Content remain local and ignored.

For larger assets on the24GB4090, `--decode-row-chunk 262144` bounds normalization, pointwise MLP, final projection, upsample gather and residual allocations, and streams large conv1 channel groups. Retired per-resolution convolution maps are removed in place from their shared cache dictionaries; rebinding one tensor's cache would leave aliases alive. This inference-only path retains the pinned weights, subdivision predictions, spatial convolution neighborhoods, normalization axes and requested resolution. Independent source review covered the operations and fresh-latent cache contract; no numerical-equivalence claim. It is not intended for training or cached encoder/decoder round trips.

RootRock completed at 1536/4K in `bellroot-kit-v1/root-rock/source-resumed4`, after the original decode and three resumed variants failed on large allocations; preserve `root-rock/source/latents.pt`. BellTree still exceeded 24GB during the final upsample neighbor-map allocation after six 1536 decode attempts. Those failed metadata files and `bell-tree/source-retry1/latents.pt` remain intact. Root explicitly informed Klaus before choosing a separate `--quality 1024_cascade` generation with 4K texture/export settings. Its geometry decoded, but the texture step initially exceeded memory. Holding the finished geometry in system RAM during texture decoding resolved this; the complete result is moved back to CUDA before export.

BellTree completed in `bellroot-kit-v1/bell-tree/source-1024-resumed` from the saved `source-1024/latents.pt`: actual 1024, 32,196 shape tokens, full 59,692,220-triangle raw geometry retained, 4K PNG maps, 308.819-second resumed decode/export run. Blender prepared 969,414 triangles with three UCX hulls at 6.978 x 6.834 x 7m. Roots occupy nearly the full base footprint, so placement must account for them. Cloister and RootRock remain 1536; lower tree voxel resolution is not asserted to have equivalent quality. The runner rejects unrequested token-driven resolution fallback. These results do not establish that arbitrary scene-scale assets fit the 4090.
