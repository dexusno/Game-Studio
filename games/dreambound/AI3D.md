...
Klaus said the work was becoming overly pedantic. All additional polishing, profiling, final reruns and new4K rendering were stopped; no workers or background applications remain active. The optional native profiling branch is disabled by default and untested. The final owner-facing result is v2 in Blender with its known seams; previous v1 engine evidence must not be relabeled as v2. Future asset work should use a bounded repair and a direct visual check, then deliver, with further validation only for an actual issue or explicit need.
