...
void ADBGameMode::TickMotionDemo(float Dt)
{
 ...
 if(!bDemoRecording){
  ...
  IFileManager::Get().MakeDirectory(*(FPaths::ProjectSavedDir()/TEXT("CombatFeelCapture")),true);
 ...
 if(DemoTime>36.f){
  ...
  if(DemoTime>38.f)FGenericPlatformMisc::RequestExit(false);
  return;
 }
 ...
 else if(DemoAction==12&&DemoTime>20){Player->UseSpecial();DemoAction=13;}
 else if(DemoAction==13&&DemoTime>22){Player->Dash();DemoAction=14;}
