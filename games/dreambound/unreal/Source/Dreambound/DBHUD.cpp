...
void ADBHUD::DrawHUD(){
 ...
 if(!G->bTitle){
  ...
  if(P){
   ...
   Label(E+TEXT(" CORE  /  R to cycle"),1430,H-133,21,Mint);
   Label(P->GetSpecialLabel(),1430,H-96,22,Ivory);
   Panel(1430,H-61,300,5,FLinearColor(0.12f,0.16f,0.17f));
   Panel(1430,H-61,300*FMath::Clamp(P->Heat/P->MaxHeat,0.f,1.f),5,P->bOverheated?FLinearColor(1.f,0.25f,0.1f):Gold);
   if(P->bOverheated)Label(TEXT("VENTING"),1745,H-76,18,Gold);
   ...
   Label(TEXT("LMB pulse  /  RMB guard  /  Shift dash"),705,H-57,18,Ivory);
   Label(TEXT("Space jump  /  Tab build  /  Esc pause"),725,H-32,17,Gold);
   if(!G->bPaused&&!G->bChoosingReward&&!G->bShowingBuild&&!G->bWon&&!G->bDefeated) {
    float X=Canvas->SizeX/2,Y=Canvas->SizeY/2;
    FLinearColor C=P->bGuarding?Mint:Ivory;
  ...
  for(TActorIterator<ADBEnemy> It(GetWorld());It;++It)if(!It->bDead&&It->RoomId==G->CurrentRoomId){
   ...
   if(It->Kind==EDBEnemyKind::Boss){
    ...
    Label(TEXT("THE BELL GUARDIAN"),640,53,22,Gold);
  ...
  if(!G->bPaused&&!G->bChoosingReward&&!G->bShowingBuild&&G->ClearedRooms.Contains(G->CurrentRoomId)){
 ...
 if(G->bTitle){
  ...
  Label(TEXT("CYBORG / PLAYABLE BETA"),115,100,20,Gold);
  Label(TEXT("BETWEEN"),108,153,79,Ivory);Label(TEXT("WORLDS"),108,230,79,Ivory);
  Label(TEXT("The places you dream about are real."),115,343,29,Ivory);
  Label(TEXT("Build a weapon worth carrying between them."),115,392,23,Mint);
  Button("Resume",G->bCanResume?TEXT("Resume expedition"):TEXT("Begin expedition"),115,475,625,70,true);
  ...
  Label(TEXT("First beta: combat, rewards and a crossing. Your playtest decides the next step."),115,H-60,16,Ivory);
 }
  ...
  Label(G->bWon?TEXT("YOU CARRIED IT THROUGH."):TEXT("CONNECTION LOST."),360,170,60,Ivory);
  Label(G->bWon?TEXT("A guardian defeated. A new world reached. Your discoveries endured."):TEXT("The expedition ended. Your learned patterns and milestones remain."),360,270,27,Mint);
  ...
  Label(TEXT("LMB / aimed pulse"),1030,255,25,Ivory);
  Label(TEXT("RMB / guard; time the opening for a deflection"),1030,304,22,Ivory);
  Label(TEXT("Q / impact or captured fire; RMB + Q / impact"),1030,353,22,Ivory);
  Label(TEXT("Shift / dash    Space / jump    R / change core"),1030,402,22,Ivory);
  Label(TEXT("E / interact    1–3 / reward    Tab / build"),1030,451,22,Ivory);
...
void ADBHUD::NotifyHitBoxClick(FName Box){
 Super::NotifyHitBoxClick(Box);auto* G=Cast<ADBGameMode>(UGameplayStatics::GetGameMode(this));if(!G)return;
