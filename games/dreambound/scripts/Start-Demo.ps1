...
$reverieExecutable=Join-Path $reverieGameRoot 'BuildOutput/CreatureMotion/Windows/Dreambound.exe'
if(-not(Test-Path -LiteralPath $reverieExecutable -PathType Leaf)){
 throw 'The CreatureMotion package is missing. Build it with scripts/Build.ps1 -Stage All.'
}
