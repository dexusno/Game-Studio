...
Windows Shipping **0.5.0-living1**: `BuildOutput/LivingWorld/Windows/Dreambound.exe`, or `scripts/Start-Demo.ps1`. Keep the entire Windows directory together. Final executable source is `877be8e120fa89bdbd345f73f245b0cf031214de`; [build identity](evidence/living-world-build.json) records hashes, import evidence, exact verification revisions and retained owner input configuration. Earlier packages are preserved.
...
3. Original creature reference images, rig specifications and reports are in art/organic-enemies. Large generated sources, finished FBX/Blender files and unchanged 4K maps remain under `<windowsOutputRoot>/organic-enemies/{briarhide,mire-seer}/finished`. Scripts/prepare_organic_enemy.py reproduces preparation from each source GLB and rig JSON. A fresh machine needs these private generated sources or regeneration; the importer fails rather than silently substituting another mesh.
...
6. Run `scripts/Build.ps1 -Stage Package`. Default output is LivingWorld. Use `-PackageConfiguration Development -OutputName LivingWorldDev` for a separate diagnostic build. This internal Shipping configuration is not an external release.
