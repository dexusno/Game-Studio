...
Open `BuildOutput/CreaturePerformance/Windows/Dreambound.exe` for the current **0.5.2-performance1** demo, or run `scripts/Start-Demo.ps1`. Creatures coil into their attacks, carry their weight through contact, regain balance after hits and collapse with separate settling movements. The Hunter reaches forward and absorbs its landing; the guardian drives its claws into the ground before rising. Your weapon carries through strikes and braces visibly during a dash. [Watch the 37-second motion review](evidence/creature-performance-review.mp4); it uses consecutive frames from the packaged game and is silent. Keep the whole Windows directory together.
