...
Current private playtest: **0.5.4-fire3**. Run `scripts/Start-Demo.ps1` or open `BuildOutput/OrganicFireCombat/Windows/Dreambound.exe`. The caster holds useful firing positions, flanks a raised shield, creates space under close pressure, reacts to visible throws and recent hits, and resumes attacking. Forward travel replaces constant sidestepping; committed casts and recovery remain punishable. Both revised faces and Klaus's fireball sound are included. [Watch the recorded fight](../../.local/organic-fire-review/caster-combat-delivery.mp4). This is a scripted player exchange with actual enemy AI; ordinary play and difficulty feedback remain open. The earlier packages remain available.
