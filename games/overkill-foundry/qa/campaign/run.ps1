param([string]$SourceRoot = (Join-Path $PSScriptRoot 'build/frozen-game'))
$ErrorActionPreference = 'Stop'
$qaRoot = $PSScriptRoot
$source = (Resolve-Path -LiteralPath $SourceRoot).Path
$qaBuild = Join-Path $qaRoot 'build'
...
function PortableLine([object]$value) {
    ([string]$value).Replace($source, '<review-source>').Replace($source.Replace('\','/'), '<review-source>').Replace($qaRoot, '<qa/campaign>').Replace($qaRoot.Replace('\','/'), '<qa/campaign>')
}
...
if ($LASTEXITCODE -ne 0) { throw 'Independent campaign configure failed.' }
& $qaCmake --build $qaBuild --config Release --parallel 4 | ForEach-Object { PortableLine $_ }
if ($LASTEXITCODE -ne 0) { throw 'Independent campaign build failed.' }
$qaExecutable = Join-Path $qaBuild 'Release/campaign_probes.exe'
