...
function PortableLine([object]$value) {
    ([string]$value).Replace($source, '<review-source>').Replace($source.Replace('\','/'), '<review-source>').Replace($qaRoot, '<qa/upgrades>').Replace($qaRoot.Replace('\','/'), '<qa/upgrades>')
}
$inputPaths = @()
foreach ($directory in @('core/include','core/src')) {
...
if ($LASTEXITCODE -ne 0) { throw 'Independent upgrade configure failed.' }
& $qaCmake --build $qaBuild --config Release --parallel 4 | ForEach-Object { PortableLine $_ }
if ($LASTEXITCODE -ne 0) { throw 'Independent upgrade build failed.' }
$qaExecutable = Join-Path $qaBuild 'Release/upgrade_probes.exe'
