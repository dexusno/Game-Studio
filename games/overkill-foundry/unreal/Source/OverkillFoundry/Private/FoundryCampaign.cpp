...
bool FFoundryCampaign::StartNew(bool bConfirmed, uint64 Seed)
{
    ...
    Combat.Selection.clear(); Combat.SpreadTargets.Empty(); Combat.CommittedEvents.clear();
...
bool FFoundryCampaign::Continue()
{
    ...
    Drawer.Empty(); Combat.Selection.clear(); Combat.SpreadTargets.Empty(); ++SceneRevision;
    bCanResumeInMemory = true;
    return true;
}
bool FFoundryCampaign::Apply(overkill::CampaignAction Action)
{
    ...
    { ++SceneRevision; Combat.Selection.clear(); Combat.SpreadTargets.Empty(); Combat.FixSelection(); }
    return true;
}
...
{
    const auto* C = Current();
    if (bPrecisionModal || !C || C->phase != overkill::CityPhase::Fight || Page != TEXT("combat") || !Drawer.IsEmpty()) return false;
