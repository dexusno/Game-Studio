...
**Design proposal, 15 September; not an owner-selected roster or tested balance.** Klaus authorized developing the enemy-behaviour example after the gap audit. Use two simple attackers beside one armoured threat to connect the selected multiple-shot, parts-only damage and stored-part rules. This is a worked paper encounter, not a playable implementation or approved demo scope.
...
- **One large shot:** in round 2 the same parts fired separately for 6, 8, 3 and 3 would deal only 3 + 5 + 0 + 0 = 8 after Plating. Combining them deals 17. This rewards assembly without adding a firing cost or shot cap.
...
**Future play question:** after seeing the displayed intents and Plating explanation, can a player explain why they split shots, combine parts or hold Shield parts, without following a prescribed sequence? If they cannot predict these consequences, revise the feedback. If the same sequence dominates different enemy patterns and available builds, revise the encounter combinations and tuning. This question does not authorize a build.

**Next discussion:** review this enemy-behaviour proposal as a concrete example. The roster, Plating buff and Charge/Blast pattern are proposals; the numbers remain beta placeholders. Continue enemy design from owner feedback without reopening the settled turn/Shield/recipe rules or expanding into an entire roster automatically.
