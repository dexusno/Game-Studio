...
**Next discussion:** review the [three-city robot roster](ENEMY-ROSTER.md) drafted from the subsequent 16 September research assignment. Its Rivet Mite and Breach Ram are proposed names for this example's Scrap Drone and Siege Robot; this document preserves the approved route and values. The approved example route and Armor name are recorded; the example does not establish a complete roster or lock numerical balance. No additional recipe-name replacements were specified in this feedback, so the existing recipe names remain unchanged.
