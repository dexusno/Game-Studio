...
| --- | ---: | --- | --- | --- |
| C1-R01 **Rivet Mite** | 6 or 7 | Attack 5 every turn. At most one live Rivet Mite per encounter. Initial placement requires a stronger companion; a lone survivor is allowed. Use 7 HP for the revised teaching fixture and 6 only where a formation explicitly specifies it; no random HP roll. | Several small shots can remove several attacks. | Original Scrap Drone from our approved example; group-pressure lesson also appears in [Nibbit formations](https://slaythespire.wiki.gg/wiki/Slay_the_Spire_2:Nibbit). |
...
| C1-R07 **Coil Nest** | 28 | Open by deploying one Rivet Mite. Thereafter alternate Attack 6 with a deployment action that supplies one mite only when none is alive and supply remains. Otherwise that action is Attack 6. Lifetime supply remains four mites total, with at most one alive at a time. | Burst the nest or control its limited support supply; pure waiting does not generate an infinite army. | [Fogmog](https://slaythespire.wiki.gg/wiki/Slay_the_Spire_2:Fogmog), without repeated resurrection or deck pollution. |
...
| C1-O03 **Split Chassis** | 48 | Attack 5 × 2; apply Thermal Runaway 2; Attack 4 × 3. On destruction releases one 7-HP Rivet Mite, shown in its inspection from the start. | Preserve damage for the visible surviving mite. Later hits aimed at the destroyed carrier are lost; the new entity requires later legal targeting. This lone death-spawn is allowed by the owner's survivor clarification. | [Phrog Parasite](https://slaythespire.wiki.gg/wiki/Slay_the_Spire_2:Phrog_Parasite). |
...
| C2-R06 **Feedback Pylon** | 34 | Raise Reactive Mesh 2 for the next player turn without attacking; next action removes Mesh and attacks for 14; next action Attack 8; repeat. | Attack during exposed windows or accept a clearly shown recoil cost. Remaining installed Shield protects against ordinary recoil during Fire; overflow reaches HP. | [Spiny Toad](https://slaythespire.wiki.gg/wiki/Slay_the_Spire_2:Spiny_Toad). |
...
| C2-O02 **Backscatter Rig** | 62 | Reactive Mesh 1 throughout the fight. Attack 3 × 4; Attack 18; gain 2 Drive Gain. | Prefer fewer direct hits or available status damage; every qualifying direct hit has a visible recoil cost to remaining Shield, then HP. | [Entomancer](https://slaythespire.wiki.gg/wiki/Slay_the_Spire_2:Entomancer); a damaging-hit reaction replaces deck interference. |
...
Helpers enter with their next-turn intent visible. A helper created in round R never acts before the enemy phase of round R + 1, including the mite released when Split Chassis dies during preparation. It can still be targeted by a later player action immediately after appearing. A destroyed summoner cancels its undeployed queue; already-deployed helpers remain and must be defeated or legitimately escape. The exception is Split Chassis's explicitly announced one-time death deployment. An encounter ends only after no live enemies remain and the player survives. A summoned entity contributes at most its assigned core allocation once, and the encounter generates its normal single recipe offer; spawning never generates extra three-recipe offers or an infinite loot loop. Initial core values are in [the beta profile](data/beta-balance-v1.json).

## Rivet Mite placement — owner decision, 20 September

Rivet Mites do not appear in pairs or as standalone opening encounters. Place one alongside a stronger chassis. At most one may be alive at a time, including Coil Nest reinforcements and Split Chassis's death-spawn. The owner accepts a lone remaining Mite as a non-blocking beta case: do not make the stronger enemy invulnerable, force the Mite to flee, despawn it or activate overtime. Its residual farming potential is a playtest concern, not a claim that the arithmetic disappeared. Reject future formations that combine suppliers in a way that can produce two live Rivet Mites. Other helper species are unaffected.

## Historical encounter pools and pacing

The table below predates the twelve-city registry. Use [CAMPAIGN-PROGRESSION.md](CAMPAIGN-PROGRESSION.md) for current placement/gates, and [BETA-TUNING.md](BETA-TUNING.md) for the initial numerical profile; this table is retained as historical formation exploration.
