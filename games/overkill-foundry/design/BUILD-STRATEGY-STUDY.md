# Overkill Foundry — build strategies and stacked ammunition
...
Recipe-design follow-up, 14 September: [the four-mercenary catalogue](RECIPE-CATALOGUE.md) turns this reference study into 600 proposed craftable recipes, with explicit parts, costs, cooldowns and effects. Its four character proposals develop the later memory/crafting rules; the earlier build examples below remain research alternatives. The catalogue's values have not been playtested.

## Direction from Klaus

**Later owner-defined rules, 13 September 2026:** [the current redesign plan](REDESIGN-PLAN.md#owner-defined-recipe-and-part-system) specifies character-specific starting weapons/core recipes, one magnet haul per turn, player-assigned furnace crafting, ammo/defence/modifier parts, and retention of unused raw resources/parts between turns. **Both consumable pools clear at fight end; retained recipes and permanent weapon upgrades persist through the current game.** Upgrades may expand recipe memory or grant fresh starting or turn-timed resources/ready parts. Recipe memory has a hard capacity, exact size later; each round the player chooses stored recipes according to their costs and availability rules. A cooldown recipe becomes usable again as soon as all counters are removed, including within the same turn, if resources suffice; each craft activates cooldown again. Recipes without a cooldown ability allow one craft per turn unless a permanent upgrade explicitly says otherwise. Other recipes may remove counters; without such effects, a two-round cooldown blocks the next two rounds. Recipe rewards normally offer one of three or a skip, with exchange available at full memory. Materials fit many recipes, and the player chooses what to craft now or later. Standalone removal has no current advantage; Klaus leaves either adding a benefit for free memory or dropping the service open. Exact costs, cooldown-counter effect activation/targets, cooldown handling at fight boundaries, turn ending and new-game/profile persistence remain open. This update governs over earlier direct-component-gathering, multiple-haul and persistence proposals below; research examples are not implemented or numerically approved.

**Prepare screen, later owner requirement:** support at least 20 applied parts per bullet and 20 per shield, showing accumulated strengths/abilities through effects information or physical additions with a summary. Hard part caps versus resource-only balance and the exact presentation remain open. A Recipe Memory button opens recipe cards for resource-based crafting, alongside the magnet and a massive scrap pile. The later 14 September requirements place gathering behind the player/gun, require enemy intent indicators during planning, designate the owner-supplied PNG as a style anchor with 16C anchoring attack perspective, and give every city distinct backdrops/surroundings. Magnet progression is separate from weapon rewards: shop-bought upgrades improve yield and/or bias resource types; special recipes supply a one-round enhancement for the next round only. These are requirements to record, not a UI implementation assignment.

**Campaign now chosen:** a mercenary defends fortified cities in an AI/robot takeover. Each city starts with a three-upgrade offer, has a 10–20-fight wave ending in a boss, and leads to a more advanced city. Three offered encounters can include regular, Officer and mystery types. Officers add a permanent system upgrade to recipe rewards; cores from victories sell for shop credits. Shops vary by character/progression and sell resources, recipes and upgrades; mysteries may offer better merchants, recipe modification or sacrifice-for-upgrade trades. Recipe strengths and common/uncommon/rare/legendary tiers improve power, functions and efficiency. The plan owns exact owner choices and open questions; do not substitute this study's older six build examples, illustrative costs or general progression proposals for them.
...
The desired reference is the strategy and progression of Slay the Spire 2: different semi-random early rewards suggest different builds, which grow more capable through subsequent choices. The original study did not request card presentation; the later owner requirement above now explicitly selects recipe cards in the Recipe Memory pop-up, without introducing deck/draw mechanics. Exact assembly rules, numbers, material supply, reset rules and reward distribution remain to be selected.
...
The owner has now selected **one magnet use per turn**. Earlier proposals about buying a further haul in the same turn are superseded; exact energy costs, capacity and replenishment are not selected. The magnet gathers source materials for furnace production, rather than directly collecting finished recipe parts. It should still visibly pull, lift and swing physical loads; the production flow must preserve the identity of what was gathered and what the furnace produced.
