...
- Shield duration is settled. Unloading before Fire is confirmed; secondary Shield/Utility/Helper activation and remaining Load controls still need exact ordering; a reset rule alone does not resolve them.

**Next:** choose the finished Utility part activation control/trigger during preparation, then other non-Ammo activation and remaining Load controls. Multiple shots and exclusive End Turn are confirmed. Global-cooling identity/balance and superseded Shield drafts remain content work. Shield reset and upgrade exceptions, the first cooldown tick, temporary/permanent claw coexistence and zero-or-one recipe rewards are settled. Continue design work; no gameplay implementation is authorized.
...
**Crafting, staging and activation are separate.** Recipe **Use** spends resources to create a part; clicking a build part stages it; **Load** prepares the selected bullet and shield and can be undone before firing. **Fire** consumes and resolves the assembled shot, then returns to preparation while combat continues. Only **End Turn** hands play to enemies. Decide when Shield secondary effects, cooling, helpers and dual-purpose parts activate, and how they are selected. The catalogue's older immediate non-Ammo activation conventions are not silently confirmed by this flow. Temporary next-round gathering recipes keep their craft-and-fit timing; permanent claw upgrades install immediately. Enemy intentions must remain consultable from the recipe view.

### Remaining turn and encounter gaps

**Prior-answer audit, 14 September:** checked the original owner messages from the preceding design task, not only the handoff. Klaus already specified Recipe **Use** as crafting into the parts pool, clicking build parts to add them automatically to bullet/shield, global cooling with no recipe target selector, and immediate crafting availability when the last cooldown counter is removed. The utility/helper cooling statement defines the effect, but does not explicitly select whether a finished utility part activates on clicking it, through Load, or through a separate action. The catalogue's immediate-use convention is an assistant-authored draft. A separate Utility **Use** button is a proposal, not a missing answer about what cooling does. Limit the next question to the activation control/trigger; do not reopen these settled mechanics or assume a new button is required.
...
**Next:** choose the finished Utility part activation control/trigger, then what Load activates and the timing of the other non-Ammo parts. Unloading before Fire is confirmed. The separate resource-budget, enemy-roster and economy gaps still need demo values. No gameplay code or new artwork is requested by this flow definition.
