...
**Owner decision:** after pressing Load, the player can unload and change the selected parts before firing. Do not ask whether Load is reversible again. Exact controls and access to Recipe/Shop while loaded remain to define alongside activation timing. Unloading an unfired assembly does not by itself authorize refunding a completed craft or undoing resolved effects; repeated loading must not duplicate parts or effect benefits.
...
Design implication to test: choose between one combined shot and sequential attacks, reacting to the first hit and retargeting remaining enemies. Splitting a build may become routinely stronger if every shot repeats free gun damage or a modifier intended for one shot. The catalogue's proposed 4 base damage illustrates this: parts adding 12 damage yield 16 in one shot, but splitting the same additive parts across two otherwise identical legal shots yields 20 before other effects. This is arithmetic under draft assumptions, not balance evidence. Review base damage, shot versus round modifiers, post-hit resource/status effects and shield activation under the selected flow. Do not silently add an energy budget, firing charge or shot cap to compensate.
...
- Shield duration is settled. Unloading before Fire is confirmed; secondary Shield/Helper activation and remaining Load controls still need exact ordering; a reset rule alone does not resolve them.
...
| Shot and impact | Show a cool, **short** firing animation in 16C. Stronger builds produce a larger boom; brief impact reactions communicate hit power/effects. Defeated robots fall into rubble with glowing cores. Consume the fired parts and, if combat continues, return to preparation to craft/build/fire again or press End Turn. There is no extra collection or round reset after a shot. |
| Enemy response | Only after **End Turn**, surviving enemies follow their visible intentions if combat remains active. Show attacks, buffs or special actions. Shield absorbs ordinary damage first; excess reaches HP. Firing alone never starts the enemy turn. |
...
**Owner-selected exclusive exit: End Turn.** This button ends the player turn whether zero, one or several shots have been fired; it does not fire a remaining bullet. The player may unload and change selected parts before firing. Unfired parts remain unused under the existing retention rule. Exact treatment of a still-loaded build at End Turn and activation of staged Shield/secondary effects remain to define; do not silently consume unfired ammunition or trigger effects twice.

**Crafting, staging and activation are separate.** Recipe **Use** pays resources and either creates a gun/Shield part or activates a Utility effect immediately with no part; clicking a build part stages it; **Load** prepares the selected bullet and shield and can be undone before firing. **Fire** consumes and resolves the assembled shot, then returns to preparation while combat continues. Only **End Turn** hands play to enemies. Direct Utility activation, including cooling, is settled. Decide when Shield secondary effects, helpers and dual-purpose build parts activate, and how they are selected. The catalogue's older Utility-part convention is superseded; other build-effect activation conventions remain draft. Temporary next-round gathering recipes keep their craft-and-fit timing; permanent claw upgrades install immediately. Enemy intentions must remain consultable from the recipe view.
...
- **Remaining Load details:** unloading and editing before Fire is confirmed. Can Recipe/Shop be reopened while loaded? What is reserved versus irreversibly activated at Load? How does End Turn behave while a bullet is loaded? End Turn is the exclusive enemy-turn trigger; multiple shots are confirmed.
...
- **Encounter/end conditions:** special-type offer distribution, boss/noncombat-Mystery routing, fleeing rewards, deaths outside the main shot and simultaneous defeat/victory. Player death ends the campaign and requires a new game; the Game Over presentation is selected. Collection recipe history survives death; other profile persistence and exact save/Game Over controls remain open.

**Next:** reconcile direct-Utility catalogue dependencies, then what Load activates and the timing of the other non-Ammo parts. Unloading before Fire is confirmed. The separate resource-budget, enemy-roster and economy gaps still need demo values. No gameplay code or new artwork is requested by this flow definition.
...
| Turn commitment | What Load activates, with unloading before Fire confirmed; access to Recipe/Shop and End Turn after loading; non-Ammo activation; round-start ordering | Load then target then Fire is selected; Fire returns to preparation; only End Turn leads to enemy actions. End Turn handles no-shot rounds. Empty-bullet Load is blocked. These remaining edges must agree with the recipe effects |
...
| Recipe memory and offers | Starting capacity is 20, subject to balancing after beta testing; initial starter set is 12 (8 shared, 4 character-specific), also subject to beta balancing; duplicate/stronger-version handling; shared or separate use limits/cooldowns; eligible reward recipes and memory replacement | The starter set grants 12 recipes within capacity 20, leaving 8 slots. Both values are provisional for beta balancing. Duplicates must not accidentally bypass the intended craft limit |
| Fight boundaries and defeat | Remaining player HP carries between fights; healing routes/tuning and character-meter resets; recipe cooldowns clear at each new fight; death ends the campaign and requires a new game; Collection recipe history survives death; other profile persistence and minimal save/continue behaviour | Both unused consumable pools already clear at fight end. HP carryover is selected; healing tuning and other profile persistence remain open; a restart must not silently undo the intended cost of losing |
...
**Selected since this review:** Load prepares the shot/shield and can be undone before Fire. Targets are selected; Fire resolves the shot and returns to preparation, allowing multiple shots. Only End Turn starts enemy actions. End Turn ends preparation without firing. Shield resets after the complete enemy turn unless a permanent upgrade explicitly says otherwise; beginning Shield grants follow the upgrade's specified each-turn, fight-start or specified-turn trigger. **Other starting defaults remain proposals:** Start the first balance experiment with the catalogue's resource-only costs and draft effect rules that do not conflict with owner decisions. Every new fight starts with all recipe cooldown counters cleared, as the owner confirmed; this is no longer a provisional reset rule. Use the selected capacity of 20, subject to balancing after beta testing, and the accepted provisional 12-recipe starter set (8 shared, 4 character-specific), also to be balanced through beta runs. These recommendations are not owner decisions or proof of balance.
...
The full upgrade/modifier catalogues, every mystery service, additional city environments, final campaign ending and any non-recipe profile progression can wait for a later increment. Recipe offers have no permanent unlock requirements. The demo needs only the upgrades/effects it actually uses. Standalone recipe removal can be left out of this sample while its value remains unresolved. If the demo instead includes city completion or mystery events, settle boss-offer handling and whether noncombat mysteries advance the wave before building those scenes.
...
| Victory and defeat | Fight ends when enemies are killed/fled or player dies; death shows Game Over with a gloating evil AI; death requires a new game; Collection recipe history survives death; fleeing rewards, other profile persistence, save/Game Over controls and final campaign victory remain open |
...
**Still open:** post-beta balancing of the initial capacity of 20 and the upgrade increments; duplicate/strength-version memory, use-limit and cooldown behaviour; exact cooldown-counter effects; and whether to retain standalone removal. Every new fight starts with all recipe cooldown counters cleared.
...
| New game / active profile | Start with the chosen character's basic gun and core recipes as previously selected. Death ends the campaign and requires a new game; profile Collection retains discovered recipe history. Seen offers count as discoveries even if skipped. Recipes require no permanent unlocks; character access and other cross-game persistence still need definition |
...
**Still open:** ingredient/output counts and costs; recipe strength/rarity tuning, duplicates and reward pools; post-beta balancing of the starting capacity of 20 / upgrade increments, cooldown-counter effect details (fight-start reset is confirmed); energy/activation rules; remaining Load controls and non-Ammo activation timing; Shield secondary effects and conflicting draft content (default reset and permanent-upgrade exceptions are settled); raw-resource/part storage limits; upgrade supply triggers/amounts; retaining or dropping standalone removal and any possible benefit for free memory; shop purchase staging/restock rules; mystery counting and final-boss choice; and profile progression/campaign ending. No discard/reshuffle system or numerical balance is selected by the reference analogy. The later owner decision sets the initial starter size to 12 (8 shared, 4 character-specific), subject to beta balancing.
...
- **Persistence:** both raw resources and crafted parts remain between turns and clear at fight end. Retained recipes and permanent system upgrades persist across fights/cities, and upgrades may expand recipe memory or grant fresh fight-start or turn-timed supplies. New-game starter kit remains character-specific; profile Collection recipe history survives death. Seen recipe offers count as discoveries even when skipped; recipes require no permanent unlocks. Character access, other profile progression and raw-resource/part storage limits remain open. Every new fight starts with all recipe cooldown counters cleared. The provisional starter set is 12 recipes (8 shared, 4 character-specific), subject to beta balancing. Starting recipe-memory capacity is 20, subject to balancing after beta testing.
- **Combat rules:** exact energy and haul costs, carrying limits and risky-pull consequences; shot-versus-round effect behaviour under confirmed multiple shots and exclusive End Turn; targeting and multi-target distribution; armour duration; effect order; and damage values. Poison, piercing, shock, explosions and other effects are possibilities, not an approved full catalogue.
- **World and progression:** the mercenary/AI-robot takeover and fortified-city campaign are now chosen. Character/city identities, total cities, exact bosses, mystery counting, three-choice handling at the boss, final campaign victory, character access, other profile progression and exact save/Game Over controls remain open. Recipes require no permanent unlocks. Death ends the current campaign and requires a new game. The five older premises are archived alternatives, not a pending prerequisite.
