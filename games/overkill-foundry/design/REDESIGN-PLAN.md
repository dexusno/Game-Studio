...
| Game Over | Player death ends the current campaign and requires a new game. Show the selected gloating evil-AI animation; do not offer a retry of the lost fight or Continue for the defeated campaign. Exact defeat lines, animation length/skip control, new-game controls and profile persistence remain to define. |
...
| Fight boundaries and defeat | Remaining player HP carries between fights; healing routes/tuning and character-meter resets; recipe cooldowns clear at each new fight; death ends the campaign and requires a new game; profile persistence and minimal save/continue behaviour | Both unused consumable pools already clear at fight end. HP carryover is selected; healing tuning and profile persistence remain open; a restart must not silently undo the intended cost of losing |
...
| Victory and defeat | Fight ends when enemies are killed/fled or player dies; death shows Game Over with a gloating evil AI; death requires a new game; fleeing rewards, profile persistence, save/Game Over controls and final campaign victory remain open |
...
**Owner decision, 15 September:** death ends the current campaign and requires a new game. The regular Game Over flow does not retry the lost fight or resume the defeated campaign; Continue must not present that campaign as active. Start New Game follows the chosen character, basic weapon and provisional starter-set rules. Keep the selected gloating evil-AI presentation. Profile Collection history and cross-game unlocks are separate decisions; this does not silently erase a profile or grant retained campaign equipment. The exact save implementation and Game Over controls remain to define. Next question: should profile Collection retain discovered recipes after death?
...
| New game / active profile | Start with the chosen character's basic gun and core recipes as previously selected. Death ends the campaign and requires a new game; Collection discovery, unlocks and other cross-game persistence still need definition |
