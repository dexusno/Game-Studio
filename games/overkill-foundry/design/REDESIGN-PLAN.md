...
Recipe memory is the current game's active crafting set. The profile's Collection records recipe discoveries and retains that history after death and across campaigns; discovery does not itself place every discovered recipe in memory or grant future-game access. Retained recipes persist across fights/cities, subject to exchanges and accepted recipe-sacrifice trades; standalone removal remains under review. Whether a removed/replaced recipe can be recovered without finding another offer remains open.
...
| New game / active profile | Start with the chosen character's basic gun and core recipes as previously selected. Death ends the campaign and requires a new game; profile Collection retains discovered recipe history. Discovery criteria, unlocks and other cross-game persistence still need definition |
