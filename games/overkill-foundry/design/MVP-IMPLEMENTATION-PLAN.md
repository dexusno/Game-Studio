...
The owner resolved recoil protection on 20 September: Shield automatically activates when firing if installed parts provide strength. The first-MVP scope remains pending; the Mite/overtime discussion has an owner disposition and is not a blocking question. The other open rows are concrete work packages. They do not require a new questionnaire before every implementation step.
...
| --- | --- | --- | --- |
| P00 | Scope, contracts and existing-pool manifest | — | Record the scope answer; apply completed timing/save and Shield/recoil contracts and beta-balance-v1; derive exact enabled content. Apply the single-Mite placement rule and record residual farming in playtests. No implementation from assumed answers. |
...
Finish P00 using the pending scope answer and the settled Shield/recoil rule. The immediate follow-up deliverable is a concrete enabled-content manifest derived from the existing framework, using the completed beta-balance-v1 fixtures. The lone-Mite farming case is accepted for beta and does not block this work. Do not ask the owner to establish progression/content pools again. The [18 timing/save traces](TIMING-AND-PERSISTENCE.md#7-worked-traces-and-future-runtime-checks) are specified and will become shared-core checks during implementation. Then request/receive the owner's next implementation goal under the existing step-by-step workflow. This planning request does not authorize starting the engine project automatically. No balance, gameplay or graphics approval is claimed by completing this document.
