...
Finish P00 using the pending scope answer and the settled Shield/recoil rule. The immediate follow-up deliverable is a concrete enabled-content manifest derived from the existing framework, using the completed beta-balance-v1 fixtures. The lone-Mite farming case is accepted for beta and does not block this work. Do not ask the owner to establish progression/content pools again. The [22 timing/save traces](TIMING-AND-PERSISTENCE.md#7-worked-traces-and-future-runtime-checks) are specified and will become shared-core checks during implementation. Then request/receive the owner's next implementation goal under the existing step-by-step workflow. This planning request does not authorize starting the engine project automatically. No balance, gameplay or graphics approval is claimed by completing this document.
