...
**There is enough settled design to plan and build the common foundations. The complete MVP boundary is not yet ready to call settled.** The original 111 recipe clarifications are closed. The general timing/persistence contract is now specified. The STS2-inspired progression/content framework and draft rosters already exist. Remaining preparation is the pending MVP-scope choice and concrete gathering/economy fixtures; translating the relevant existing pools into executable content is implementation work. Runtime verification remains implementation work. No game implementation starts as part of this planning assignment.
...
| Existing content framework; implementation still required | The campaign registry already assigns opening/middle/late robot formations, Officers and bosses across twelve cities. Recipe starters/shared-character rarity pools and filtered upgrade/Mayor pools already exist | Derive the build manifest from those sources for the chosen demo boundary and implement its effects. This is not a missing progression framework or a fresh owner content-selection exercise. Individual Mystery outcomes still need authoring; offer weights and numerical strength need beta tuning. |
...
What remains is implementing the relevant effects/data, writing individual Mystery outcomes, tuning drop/offer weights and checking whole-city balance. An incomplete runtime does not mean the design framework is missing.
...
Finish P00 using the pending scope answer and the settled Shield/recoil rule. The immediate follow-up deliverable is a concrete enabled-content manifest derived from the existing framework, plus its gathering/economy fixtures. Do not ask the owner to establish progression/content pools again. The [18 timing/save traces](TIMING-AND-PERSISTENCE.md#7-worked-traces-and-future-runtime-checks) are specified and will become shared-core checks during implementation. Then request/receive the owner's next implementation goal under the existing step-by-step workflow. This planning request does not authorize starting the engine project automatically. No balance, gameplay or graphics approval is claimed by completing this document.
