...
**Saved-Utility bonus replacements — owner-authorized analogous fixes, 17 September:** Pocket Screen counts ordinary parts in reserve saved from earlier rounds, using its selected 0/1/2+ tiers. Twin Coolant and Packed Lunch use the same real-part dependency: their bonus requires at least two ordinary parts saved from earlier rounds still in reserve when the recipe is used. Count without consuming the parts; newly made parts do not qualify. Each Utility resolves immediately and produces no Utility item. Twin Coolant retains global cooling, Corrosion 3 to one chosen enemy when eligible, and its requirement that a recipe already be cooling before Use. Packed Lunch retains 5 Bolt HP restored, or 10 with the bonus, including while disabled. Material costs and cooldowns are unchanged. Apply settled solutions to equivalent conflicts without repeating the same design question.

**Split Utility activations combined — owner-selected, 17 September:** Service Pair immediately repairs 6 Bolt HP on recipe Use, only while Bolt is active. Split Battery immediately grants 4 Charge on recipe Use. These totals replace two 3-HP tabs and two 2-Charge cells respectively; neither recipe produces inventory items or separate activations to save. Existing HP/Charge limits still apply. Costs, cooldowns and normal use limits are unchanged.

**Superseded dependencies, revise before use:** IV105/AD097 consume stored Utility parts; NO094 requires a part-based cooling trigger and needs reconciliation with direct Utility cooling. SH071/SH076/SH102/SH118, MA107/MA119, IV118 and NO107/NO120 explicitly create or copy parts through Utility effects: they require classification/effect review under the owner's no-part rule, not automatic approval as exceptions. Their original effect amounts remain drafts; no replacement reward or new category is invented here. References elsewhere to counting crafted recipes, part activations or part-only payments also require an explicit reading under the new distinction.
...
**Four small effect definitions used in the rows:** Weaken's enemy behaviour below is owner-selected on 17 September; the other definitions remain draft content. These support reading the recipes and do not constitute the complete buff catalogue.
...
| Weaken N | Reduce each hit of that enemy's attacks by N damage, minimum zero. Hits do not consume it. Reduce N by 1 once at the end of the complete enemy phase, including rounds with no attack; remove it at zero. |

**Weaken — owner-selected enemy rule, 17 September:** Weaken N reduces each hit of that enemy's attacks by N damage, minimum zero, throughout the round. Hitting does not consume Weaken. After the complete enemy phase, reduce its amount by 1 once, including rounds when it did not attack; remove it at zero. Multiple hits/actions do not cause extra decay. Thus an enemy with a normal 10-damage hit and Weaken 3 deals 7 this round, 8 next round at Weaken 2, 9 at Weaken 1, then 10 when cleared. A 6 + 6 attack under Weaken 3 deals 3 + 3 before other defences. Existing additive applications and explicit cleansing still apply. This replaces the old next-attack-total reduction and immediate clearing. It does not introduce a percentage modifier or change whole-number rounding.
...
**When these effects are on the player:** Corrosion removes HP at the start of planning, after the prior enemy-end Shield reset and before fresh start-of-turn benefits. Burn damages Shield then HP after the player's action phase, even if no shot was fired. Each ticks once and then falls by 1. Mark adds its amount to the next enemy attack's total damage, then clears; for a multi-hit attack add it to the first hit. For the player's draft counterpart, Weaken subtracts its current amount from each main shot's calculated damage before target bonuses, minimum zero; Fire does not clear it. Reduce it by 1 once when the player ends the action phase, even after zero shots, and remove it at zero. Enemy-applied Weaken therefore survives to the next player action phase without an immediate enemy-end tick. This mirrors the selected persistent enemy effect while retaining the player's existing main-shot calculation scope. These are draft definitions so cleansing and protection recipes have a clear use; enemy move lists remain later work.
...
| IV099 | Twin Coolant | Immediate effect: Split Coolant Tube (no part) | Utility | 1 Copper + 2 Glass + 1 Circuit | 3 | Cannot Use unless at least one recipe is already cooling. On recipe Use, check whether at least two ordinary parts currently in reserve were saved from an earlier round, without consuming them. Remove 1 cooldown counter from every currently cooling recipe. If the saved-part condition was met, also apply Corrosion 3 to one chosen living enemy now. This Utility activates immediately and produces no Utility part. |
...
| AD040 | Service Pair | Immediate effect: Service Tabs (no part) | Utility | 2 Copper + 1 Iron | 1 | On recipe Use, immediately repair 6 Bolt HP. Can only be used while Bolt is active. This Utility produces no part or separately stored activation. |
...
| AD062 | Packed Lunch | Immediate effect: Stored Service Pack (no part) | Utility | 2 Iron + 1 Copper | None | On recipe Use, restore 5 Bolt HP, including while disabled. Restore 10 instead if at least two ordinary parts currently in reserve were saved from an earlier round. The counted parts are not consumed. This Utility activates immediately and produces no Utility part. |
...
| NO055 | Split Battery | Immediate effect: Small Cells (no part) | Utility | 2 Copper + 1 Glass | None | On recipe Use, immediately gain 4 Charge. This Utility produces no part or separately stored activation. |
