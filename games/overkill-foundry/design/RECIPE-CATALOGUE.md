...
**Multiple-shot reconciliation:** a shot is not a new round. Firing does not repeat collection, restore recipe uses, tick cooldowns or clear Shield. The earlier single-main-shot convention is superseded. The earlier turn-flow clarification alone left recipe rows unchanged; the duration decision below now clarifies six Utility effects. Review remaining shot-versus-round modifiers, draft 4 base gun damage, post-hit resource/status gains and non-Ammo activation under the new flow. Do not silently apply an effect written for one shot to every shot or introduce a firing cost/cap.
...
**Utility activation settled:** Recipe Use directly activates Utility effects, including cooling; no part or second control exists. Draft Helper/Modifier/gathering classifications and gun/Shield secondary-effect timing still need reconciliation. **Owner decision, 15 September:** Fire activates the prepared shield, which stays active through the enemy turn and resets at its end under the existing permanent-upgrade exceptions. Crafting, staging and Load do not activate it. **Further owner decision:** on a zero-shot turn, End Turn activates the prepared shield before enemy actions without ammunition or Load. **Further owner decision:** newly activated Shield adds to the remaining active Shield, including across multiple shots; 8 + 5 = 13 if none was lost or spent. End Turn activation of new Shield prepared after the last shot, used-part accounting, secondary-effect ordering and remaining Load controls remain open; unloading and editing before Fire is confirmed. Permanent claw upgrades activate on purchase/acceptance and never enter the parts pool. Temporary gathering recipes remain the next-round-only craft-and-fit exception.
...
| Shield | Craft and stage in the shield build, then activate the prepared shield when Fire is pressed, or with End Turn before enemy actions if no shot was fired that turn. Protection stays active through the enemy turn and resets at its end under the existing permanent-upgrade exceptions. Immediate planning-phase activation is superseded. The zero-shot End Turn route needs no ammunition or Load. New Shield from later shots adds to the remaining active Shield. End Turn activation of new Shield prepared after the last shot, used-part accounting and secondary-effect ordering remain to define. |
...
| Utility | Activate the recipe directly on Use, paying its material cost and applying its use/cooldown rules. No part is produced, stored, loaded or consumed. Stated ongoing/delayed effects are activated now and follow their triggers; conflicting draft outputs require revision below. |
...
The selected round is collection, preparation with zero or more separately built shots, End Turn, surviving enemies' actions and the enemy-turn-end reset/tick. Fire returns to preparation while combat continues. Helpers and delayed parts may make support hits; these are distinct from the assembled main shots. Fire activation of the prepared shield and End Turn activation on a zero-shot turn are selected. New Shield from later shots adds to the remaining active Shield. The after-last-shot End Turn case, used-part accounting, secondary effects, other non-Ammo timing and remaining round-boundary ordering stay open. Enemy intent remains visible while choosing attack and defence.
...
| SH034 | Early Cover | Immediate effect: Delayed Plate Pack (no part) | Utility | 1 Iron + 1 Copper | None | At the start of next turn, gain 8 Shield. It expires at the following turn's start unless retained. |
...
| IV071 | Pocket Screen | Immediate effect: Dust Screen (no part) | Utility | 1 Carbon + 1 Copper | None | **Superseded Utility dependency; revise before use.** Apply Weaken 2 to every living enemy now. If this part was saved, apply Weaken 3 instead. |
