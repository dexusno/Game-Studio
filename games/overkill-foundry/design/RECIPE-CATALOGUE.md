...
**Utility activation settled:** Recipe Use directly activates Utility effects, including cooling; no part or second control exists. Draft Helper/Modifier/gathering classifications and gun/Shield secondary-effect timing still need reconciliation. **Owner decision, 15 September:** Fire activates the prepared shield, which stays active through the enemy turn and resets at its end under the existing permanent-upgrade exceptions. Crafting, staging and Load do not activate it. **Further owner decision:** on a zero-shot turn, End Turn activates the prepared shield before enemy actions without ammunition or Load. Multiple-shot Shield accounting, secondary-effect ordering and remaining Load controls remain open; unloading and editing before Fire is confirmed. Permanent claw upgrades activate on purchase/acceptance and never enter the parts pool. Temporary gathering recipes remain the next-round-only craft-and-fit exception.
