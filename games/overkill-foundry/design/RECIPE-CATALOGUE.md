...
**Part resale, owner decision 15 September:** unused crafted parts can be sold for 50% of their crafting ingredients' credit value. For a single-output craft, the unrounded price is `0.5 × sum(ingredient quantity × current shop resource unit price)`. **Owner-selected valuation reference:** use the shop's current resource prices at sale time. Printed recipe material counts are not themselves credit prices. Rounding and cost allocation for copied/generated or multi-output parts remain open. Utility recipes create no saleable Utility part. Recipe rows and their costs remain unchanged.
