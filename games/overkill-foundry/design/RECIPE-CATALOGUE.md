...
**Part resale, owner decision 15 September:** unused crafted parts can be sold for 50% of their crafting ingredients' credit value. For a single-output craft, the sale price is `floor(0.5 × sum(ingredient quantity × current shop resource unit price))` credits. **Owner-selected rounding:** round the resulting sale price down to whole credits, so 2.5 becomes 2; apply this after summing the ingredient value and taking half. **Owner-selected valuation reference:** use the shop's current resource prices at sale time. Printed recipe material counts are not themselves credit prices. **Owner-selected identical-output allocation:** when one craft produces N identical parts, divide its ingredient value equally among those N parts before applying the 50% factor and rounding down per part. The sale price of each is `floor(0.5 × sum(ingredient quantity × current shop resource unit price) / N)` credits. Ingredients worth 8 credits producing two parts yield 2 credits per part. Use the number produced by that craft, not the count remaining in inventory. **Owner-selected copy resale:** a copied part retains the original's allocated ingredient basis for sale pricing and is worth the same amount under the same current shop resource prices. A currently 4-credit part has a currently 4-credit copy. Do not dilute the original allocation between original and copy or apply another 50% discount to the original's sale value. The copying recipe's costs and effect rules remain unchanged. Attribution for non-copy generated parts and mixed-output crafts remains open. Utility recipes create no saleable Utility part. **Owner decision, 15 September:** raw materials cannot be sold to the shop; raw-material purchases remain available. This does not change crafted-part or energy-core sales. Recipe rows and their costs remain unchanged.
...
Copies arrive as newly received parts: their age starts again, and they keep the source recipe's rarity and identity. **Owner decision, 15 September:** they also retain the original part's allocated ingredient basis for resale, so original and copy sell for the same amount at the same current shop resource prices. Generated subparts inherit their creator recipe's rarity; copying one reproduces that subpart, not its generator. If a generator fixed its damage or Shield when creating it, that fixed value is copied. Later attached bonuses are not. Per-recipe healing or trigger caps are shared across all its outputs and copies.
...
These identities and traits remain proposed ways to make the characters feel different from the first fight. **Parts-only damage reconciliation, 15 September:** Hot Barrel and Charged Barrel currently grant shot damage from a character/weapon trait rather than a bullet part; those damage-source drafts conflict with the new owner rule and require revision before use. Do not keep their independent damage grants or silently turn a Utility into a part. Other trait/effect interactions still need their individual review; existing immediate-Utility use and selected bonus durations remain recorded. Klaus accepted the listed starter sets on 15 September as a provisional beta-balancing baseline: 12 recipes per character, 8 shared plus 4 exclusive. This does not establish final character identities or tested recipe balance.
...
- A Plain Slug and a saved Settled Charge supply 6 + 15 = 21 damage. Layered Charge sees two Ammo parts, so adds 16%: floor(21 × 1.16) = 24 main damage. Wide Burst gives each extra enemy floor(24 × 0.5) = 12 damage. Its explicit draft effect does not repeat the main target's payloads.
