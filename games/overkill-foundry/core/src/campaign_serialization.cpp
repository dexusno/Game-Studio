...
template<class A>void field(A&,OwnedUpgrade&);template<class A>void field(A&,ProfileFacts&);template<class A>void field(A&,CampaignReceipt&);template<class A>void field(A&,Event&);
...
template<class A>void field(A& a,RouteOffer& x){fields(a,x.id,x.kind,x.formation,x.mystery,x.district,x.encounterKey,x.encounterSeed,x.binderDefaultSeen);}
template<class A>void field(A& a,RouteNode& x){fields(a,x.offers,x.selected,x.resolved);}
template<class A>void field(A& a,CityRoute& x){fields(a,x.seed,x.rng.state,x.nextOfferId,x.position,x.officer,x.mystery,x.nodes,x.officerOrder,x.district,x.previousFormation,x.binderDefaultSeen);}
...
template<class A>void field(A& a,RewardEntry& x){fields(a,x.id,x.kind,x.claim,x.choices,x.cores,x.credits);}
template<class A>void field(A& a,OwnedUpgrade& x){fields(a,x.id,x.order,x.recipeCopy,x.charges,x.campaignCount,x.fightCount,x.roundCount,x.values);}
template<class A>void field(A& a,ProfileFacts& x){fields(a,x.recipes,x.unlocked,x.cityClearReceipts);}
template<class A>void field(A& a,Event& x){fields(a,x.id,x.parent,x.subject,x.target,x.type,x.amount,x.secondary);}
...
template<class A>void visit(A& a,Campaign& x){
    fields(a,x.rulesVersion,x.contentVersion,x.manifestHash,x.runId,x.seed,x.nextId,x.nextTransaction,x.phase,x.rng.state,x.profile,x.fight,x.route,
           x.memorySlots,x.shopGeneration,x.cores,x.upgrades,x.everAcquired,x.mayorOffers,x.mayor,x.shop,x.eventShop,x.rewards,x.recipeWindow,x.pendingRecipeChoice,x.skipConfirmation,x.preStart,x.entry,x.receipts);
}
...
void validate(const Campaign& c,bool allowEntry){
    ...
    std::set<Id> productIds,rewardIds,coreIds,orders,receipts;std::set<std::string> upgradeIds;
    ...
    for(const auto& upgrade:c.upgrades){
