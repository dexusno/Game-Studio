"""Preserve the initial executed UGS-085 failure before its authorized repair."""
import hashlib
import json
...
def main():
    ...
    archive = BUILD / "history" / ("ugs085-failure-" + identity[:16])
    if archive.exists():
    ...
    assert run.returncode == 1 and "FAIL upgrade:UGS-085:" in log
    ...
    (evidence / "ugs085-failure.json").write_text(json.dumps(record, indent=2, sort_keys=True) + "\n", encoding="utf-8")
